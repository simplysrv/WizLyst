...
function submitPost($category, $subcategory, $locality, $city, $state, $country, $price, $free, $negotiable, $desc, $title, $uid, $img1, $img2, $img3, $img4, $img5){
	...
	`Post_id` ,`Post_category` ,`Post_subcategory` ,`Post_locality` ,`Post_city` ,`Post_state` ,`Post_country` ,`Post_title` ,`Post_desc` ,`Post_price` , `Post_free`, `Post_negotiable`, `Post_img1` , `Post_img2` ,`Post_img3` ,`Post_img4` ,`Post_img5` ,`Post_uid` ,`Post_timestamp`)
	VALUES (
	'',  '$category',  '$subcategory',  '$locality',  '$city',  '$state',  '$country',  '$title',  '$desc',  '$price',  '$free', '$negotiable', '$img1',  '$img2',  '$img3',  '$img4',  '$img5',  '$uid', CURRENT_TIMESTAMP);";
...
function recentPost($cat, $locType, $locName){
	$connection = db_connect();
	if($cat == ""){
		$sql = "SELECT * FROM  `FlickBazar_Post` ";
		
		if($locType == "city" && $locName != ""){
			$sql = $sql."WHERE `Post_city` = '$locName' ";
		}else if($locType == "state" && $locName != ""){
			$sql = $sql."WHERE `Post_state` = '$locName' ";
		}
		
		$sql = $sql."ORDER BY `Post_timestamp` DESC LIMIT 5";
		
	}else{
		$sql = "SELECT * FROM  `FlickBazar_Post` WHERE  `Post_category` = '$cat' ";
...
function stringSearch($cat, $subcat, $query, $start, $limit, $sort_type, $sort_direction, $locType, $locName){
	...
	$sql = "SELECT * FROM  `FlickBazar_Post` WHERE `Post_title` LIKE '%".$query."%' ";
...
function searchList($cat, $subcat, $start, $limit, $sort_type, $sort_direction, $locType, $locName){
	...
	if($cat == ""){
		$sql = "SELECT * FROM  `FlickBazar_Post` WHERE  `Post_category` LIKE '%' ";
	}else{
		$sql = "SELECT * FROM  `FlickBazar_Post` WHERE  `Post_category` = '$cat' ";
	}
...
function viewedPost($cat, $locType, $locName){
	...
	if($cat == ""){
		...
		$sql = $sql."ORDER BY `Post_view` DESC LIMIT 3";
...
function getSameUserPost($user_id){
	$connection = db_connect();
	$sql = "SELECT * FROM  `FlickBazar_Post` WHERE  `Post_uid` = $user_id ORDER BY RAND() LIMIT 4";
	$result = mysqli_query($connection,$sql);
	db_close($connection);
	return $result;
}
